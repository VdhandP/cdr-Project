...
### For creating necessary global data variables
def load_data():
  ...
  # pandas dataframe creation section
  global call_data
  ...
  # drop down lists of dictionaries section
...
### For creating a link to directly open a (default)browser window
def open_browser():
  webbrowser.open_new('http://127.0.0.1:8050/')



### For Webpage UI LAYOUT 
def create_app_ui():
    
    # creating dropdowns for call analytics tab
    start_date_drop= dcc.Dropdown(
    ...
    group_drop= dcc.Dropdown(
                  id='group-dropdown', 
                  placeholder = "Select group here",
                  multi = True 
    )
    
    end_date_drop= dcc.Dropdown(
    ...
    html.H1('CDR Analysis with Insights', id='Main_title',style={'padding-left':'35%'}),
    
    dcc.Tabs(id="Tabs", value="tab-1",children=[
    
    # 1st tab section
    dcc.Tab(label="Call Analytics tool" ,id="Call Analytics tool",value="tab-1",style=\
            {'backgroundColor':'DARKGRAY'}, children = [
    html.Br(),
    html.Br(),
    
    html.Div([dbc.Row([dbc.Col(id='start_date_drop', children=[start_date_drop], md=3), dbc.Col(id='group_drop', \
                                                                        children=[group_drop], md=3)]\
                            ,style={'width':'175%'}),
   
    dbc.Row([dbc.Col(id='end_date_drop', children=[end_date_drop], md=3), dbc.Col(id='report_type_drop',\
                    children=[report_type_drop], md=3)]
                            ,style={'width':'175%'})],style={'backgroundColor':'salmon','padding-left':'12.5%',\
                                                             'padding-top':'1%','height':100})]),
    # 2nd tab section
    dcc.Tab(label = "Device Analytics tool", id="Device Analytics tool", value="tab-2",style=\
            {'backgroundColor':'DARKGRAY'}, children = [            
    html.Br(),
    ...
    # 3rd tab section
    dcc.Tab(label = "Service Analytics tool", id="Service Analytics tool", value="tab-3",style=\
            {'backgroundColor':'DARKGRAY'}, children = [            
    html.Br(),
    dcc.Dropdown(
...
### For creating cards used in the call analytics tool tab
...
### For creating dataframe for the pie chart in device analytics tool
...
def count_devices(data):
    ...
    # Creating Datarame for the pie chart
...
### Callback of the page for visualization-object like bar graph, cards, tables, pie charts covering all tabs
...
def update_app_ui(Tabs, start_date, end_date, group, report_type,device_date,service_date):
    ...
    # Confirmatory section for input values
    ...
    # preparing output for the 3 tabs according to their tab values
    if Tabs == "tab-1":
        ...
        card_6 = create_card(title, content, "primary")
          ...
          # cards arrangement into rows and columns    
        ...
        datatable_data = call_analytics_data.groupby(["Group", "UserID", "UserDeviceType"])["Call_Direction"].\
            value_counts().unstack(fill_value = 0).reset_index()
        if call_analytics_data["Missed Calls"][call_analytics_data["Missed Calls"]==19].count()!=0:
            datatable_data["Missed Calls"] = call_analytics_data.groupby(["Group", "UserID", "UserDeviceType"])\
                ["Missed Calls"].value_counts().unstack()[19]
        else:
    ...
    # Output for device analytics tool
    ...
    # Output for service analytics tool
        ...
        if service_date is None or service_date == []:
            ...
            service_analytics_data = service_data["FeatureName"][service_data["FeatureEventDate"].isin(service_date)].\
                value_counts().reset_index(name = "Count")
        
        ## pie chart for service analytics tool
        fig = px.pie(service_analytics_data, names = "index", values = "Count",color = "index",hole=.3,\
                     template="plotly_dark")
        
        fig.update_layout(autosize=False,
                          margin=dict(l=0, r=0, t=10, b=100),height=800,paper_bgcolor='rgb(50, 50, 50)'
                          )
        fig.update_traces(textposition='outside', textinfo='percent+label',textfont_size=10)
        return dcc.Graph(figure = fig)
...
### Callback of the page for group dropdown in call analytics tool tab
...
### For the Callback output of group dropdown in call analytics tool tab

def update_groups(start_date, end_date): 
    
    # list of dictionaries output for the options property of the group drop down
...
def main():
    # initializing necessary global data variables
    load_data()
    
    # opening default browser window
    open_browser()
    
    # assigning layout to dash layout object and creating a title for the app
    ...
    # confirmatory output on script closing
    print("This would be executed only after the script is closed")
    
    # returning all global variables to None
    app = None
    project_name = None
